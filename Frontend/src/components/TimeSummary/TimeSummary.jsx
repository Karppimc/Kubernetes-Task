    ...
    timestamps
      ...
      .forEach((timestamp) => {
        ...
        if (timestamp.type === 0) {
          ...
          taskTimes[taskId].totalTime +=
            (timestampDate - taskTimes[taskId].lastStart) / (1000 * 60 * 60);
      ...
      .forEach((timestamp) => {
        ...
        task.tags.split(',').forEach((tagId) => {
          ...
          if (timestamp.type === 0) {
            ...
            tagTimes[tagId].totalTime +=
              (timestampDate - tagTimes[tagId].lastStart) / (1000 * 60 * 60);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const timestampsResponse = await fetch('/api/timestamps');
